---
description: Fix a bug with minimal context — loads only the domain section relevant to the affected file. Optionally creates a bug spec for traceability.
arguments: Bug description and/or file path(s). Add --trace to create a bug spec in specs/bugs/.
output: Code fix + explanation. With --trace: also creates specs/bugs/<slug>/spec.md and updates .ai/current.
---
