---
description: Validate that the current implementation satisfies the spec's acceptance criteria. Reads .ai/current automatically.
arguments: (optional) Path to spec directory. If omitted, reads from .ai/current.
output: Review report — green / amber / red per acceptance criterion
---
...
- Coverage gap: <which files/paths are uncovered>
